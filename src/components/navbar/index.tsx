        ...
        {navMapData.map((item, index) => (
          ...
          >
            ...
            {selectedOption === item.name && item.options && (
              <div className="absolute left-0 top-6 bg-white dark:bg-black rounded-md p-7 flex flex-col gap-2 z-50 min-w-[200px]">
                {item.options.map((option, index) => (
                  <div
                    key={index}
                    className="relative w-fit whitespace-nowrap cursor-pointer group/option hover:text-gray-600 text-xs"
                    onClick={() => {
                      navigate(
